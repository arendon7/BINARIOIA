...
if [[ -z "$PY" ]]; then
  /usr/bin/osascript -e 'display dialog "Binario IA no encontró Python. Reinstala el runtime desde el instalador FULL." buttons {"OK"} default button "OK" with icon caution' 2>/dev/null || true
  exit 10
fi
load_secret() {
