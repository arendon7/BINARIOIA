...
from common import project_storage
...
def data_root()->Path:
    # Compatibility only. Canonical state lives inside each Project Center folder.
...
def legacy_project_dir(pid:str)->Path:return legacy_data_root()/_safe(pid)

def _migrate_if_needed(pid:str)->dict:
    pid=_safe(pid);target=project_storage.app_state_dir(pid,APP_ID);legacy=legacy_project_dir(pid)
    if legacy.is_dir() and not (target/"project.json").exists():
        return project_storage.migrate_legacy_state(pid,APP_ID,legacy)
    return {"ok":True,"copied":[],"legacy":str(legacy),"target":str(target)}

def project_dir(pid:str)->Path:
    pid=_safe(pid);_migrate_if_needed(pid);return project_storage.app_state_dir(pid,APP_ID)
def project_path(pid:str)->Path:return project_dir(pid)/"project.json"
def versions_dir(pid:str)->Path:
    p=project_dir(pid)/"versions";p.mkdir(parents=True,exist_ok=True);return p
def export_dir(pid:str)->Path:return project_storage.app_export_dir(_safe(pid),APP_ID)
def training_dir(pid:str)->Path:return project_storage.app_training_dir(_safe(pid),APP_ID)
def activity_path(pid:str)->Path:return project_storage.app_log_path(_safe(pid),APP_ID)
...
def default_project(name:str="Nuevo agente", business_name:str="")->dict:
    ...
    if base:project_center.update(base["id"],{"name":name or "Nuevo agente","metadata":{**(base.get("metadata") or {}),"studio":"agent-studio-r27","storage":"canonical-project"}})
    else:base=project_center.create(name or "Nuevo agente",APP_ID,metadata={"studio":"agent-studio-r27","storage":"canonical-project"})
    pid=base["id"]
    row={
      ...
      "flow":{"nodes":[],"edges":[]},"model":{"profile":"balanced","task":"conversation","last_route":None},
    ...
    row["flow"]=build_flow(row);save(row,snapshot=False);snapshot(pid,"created");return row
...
def build_flow(row:dict)->dict:
    ...
    for it in row.get("intents",[]):
        nodes.append({"id":it["id"],"type":it.get("action","message"),"label":it.get("label",it["id"]),"text":it.get("response","")});edges.append({"from":"router","to":it["id"],"label":it.get("label","")})
    ...
    edges += [{"from":"router","to":"fallback","label":"sin match"},{"from":"fallback","to":"router"}];return {"nodes":nodes,"edges":edges}

def save(row:dict,snapshot:bool=False)->dict:
    pid=_safe(row["id"]);row["updated_at"]=now();p=project_path(pid);tmp=p.with_suffix('.tmp');tmp.write_text(json.dumps(row,indent=2,ensure_ascii=False),encoding='utf-8');tmp.replace(p)
    try:
        global_row=project_center.get(pid) or {};meta=dict(global_row.get("metadata") or {});meta.update({"studio":"agent-studio-r27","storage":"canonical-project","app_state_path":str(p)})
        project_center.update(pid,{"name":row.get("name"),"status":"active" if row.get("status")!="archived" else "archived","current_step":row.get("current_step"),"metadata":meta})
...
def get(pid:str)->dict|None:
    try:
        p=project_path(pid);return json.loads(p.read_text(encoding='utf-8')) if p.exists() else None
    except Exception:return None

def list_projects()->list[dict]:
    out=[]
    for global_row in project_center.list_projects(app_id=APP_ID,include_archived=True):
        r=get(str(global_row.get('id') or ''))
        if not r:continue
...
def snapshot(pid:str,label:str="snapshot")->dict:
    ...
    vdir=versions_dir(pid);stamp=datetime.now().strftime('%Y%m%d-%H%M%S-%f');path=vdir/f'{stamp}-{re.sub(r"[^a-z0-9-]+","-",label.lower()).strip("-") or "snapshot"}.json';path.write_text(json.dumps(row,indent=2,ensure_ascii=False),encoding='utf-8')
    meta={"id":path.stem,"label":label,"created_at":now(),"path":str(path)};versions=list(row.get('versions') or []);versions.append(meta);row['versions']=versions[-50:];save(row,snapshot=False);return meta

def list_versions(pid:str)->list[dict]:row=get(pid);return list(reversed((row or {}).get('versions') or []))
def log_event(pid:str,event:str,detail:dict|None=None)->dict:
    row={"timestamp":now(),"event":event,"detail":detail or {}};p=activity_path(pid);p.parent.mkdir(parents=True,exist_ok=True)
    with p.open('a',encoding='utf-8') as f:f.write(json.dumps(row,ensure_ascii=False)+'\n')
    return row

def activity(pid:str,limit:int=100)->list[dict]:
    p=activity_path(pid);rows=[]
    if p.exists():
