...
APP_ID='04-auditoria-youtube'; STUDIO='YouTube Studio'
def now(): return datetime.now(timezone.utc).isoformat()
def legacy_root():
    return Path(os.environ.get('BINARIO_YOUTUBE_STUDIO_HOME') or (Path.home()/'Documents'/'Binario IA'/'YouTube Studio')).expanduser()
def data_root():
...
def create(name='Nuevo canal'):
    b=native_context.claim_runtime_project(APP_ID)
    if b and path(b['id']).exists():b=None
    if b:project_center.update(b['id'],{'name':name,'metadata':{**(b.get('metadata') or {}),'studio':'youtube-studio-r19'}})
    else:b=project_center.create(name,APP_ID,metadata={'studio':'youtube-studio-r19','storage':'canonical-project'})
    r={'schema':'sbia-youtube-studio-1.0','id':b['id'],'global_project_id':b['id'],'name':name,'status':'active','current_step':'channel','created_at':now(),'updated_at':now(),'profile':{'channel_name':'','positioning':'','audience':'','offer':'','subscribers':0,'avg_views':0,'posts_per_week':3},'content':[],'analysis':{'status':'draft','scores':{},'pillars':[],'legacy':None,'reviewed':False,'review_notes':'','readiness':0},'ideas':[],'calendar':[],'experiments':[],'performance':[],'handoffs':{},'exports':[],'versions':[]};save(r);snapshot(r['id'],'created');return r
def save(r):
    ...
    try: project_center.update(r['id'],{'name':r.get('name'),'status':r.get('status','active'),'current_step':r.get('current_step'),'metadata':{'studio':'youtube-studio-r19','storage':'canonical-project','items':len(r.get('content') or []),'readiness':r.get('analysis',{}).get('readiness')}})
